...
namespace ServvistaWebAppAPI.Services
{
    public class ServiceScheduleService : IServiceSchedule
    {
        ...
        public async Task<List<ServiceVisitMonthlyInfo>> GetMonthlyVisits(string techCode)
        {
            ...
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                (
                    SELECT
                        MACHINE_REF,
                        'EXPT_SV1' AS MatchedColumn,
                        CONVERT(char(10), EXPT_SV1, 120) AS MatchedDate,
                        CASE
                            WHEN SV1 IS NOT NULL THEN 'COMPLETED'
                            ELSE 'PENDING'
                        END AS VisitStatus
                    FROM dbo.TBL_SERVICE_SCEDULE_UPDATE
                    WHERE TECH_CODE = @techcode
                      AND EXPT_SV1 BETWEEN @firstdaymonth AND @lastdayofmonth
                    ...
                    SELECT
                        ...
                        CONVERT(char(10), EXPT_SV2, 120),
                        CASE
                            WHEN SV2 IS NOT NULL THEN 'COMPLETED'
                            ELSE 'PENDING'
                        END
                    FROM dbo.TBL_SERVICE_SCEDULE_UPDATE
                    WHERE TECH_CODE = @techcode
                      AND EXPT_SV2 BETWEEN @firstdaymonth AND @lastdayofmonth
                    ...
                    SELECT
                        ...
                        CONVERT(char(10), EXPT_SV3, 120),
                        CASE
                            WHEN SV3 IS NOT NULL THEN 'COMPLETED'
                            ELSE 'PENDING'
                        END
                    FROM dbo.TBL_SERVICE_SCEDULE_UPDATE
                    WHERE TECH_CODE = @techcode
                      AND EXPT_SV3 BETWEEN @firstdaymonth AND @lastdayofmonth
                    ...
                    SELECT
                        ...
                        CONVERT(char(10), EXPT_SV4, 120),
                        CASE
                            WHEN SV4 IS NOT NULL THEN 'COMPLETED'
                            ELSE 'PENDING'
                        END
                    FROM dbo.TBL_SERVICE_SCEDULE_UPDATE
                    WHERE TECH_CODE = @techcode
                      AND EXPT_SV4 BETWEEN @firstdaymonth AND @lastdayofmonth
                    ...
                    SELECT
                        ...
                        CONVERT(char(10), EXPT_SV5, 120),
                        CASE
                            WHEN SV5 IS NOT NULL THEN 'COMPLETED'
                            ELSE 'PENDING'
                        END
                    FROM dbo.TBL_SERVICE_SCEDULE_UPDATE
                    WHERE TECH_CODE = @techcode
                      AND EXPT_SV5 BETWEEN @firstdaymonth AND @lastdayofmonth
                    ...
                    SELECT
                        ...
                        CONVERT(char(10), EXPT_SV6, 120),
                        CASE
                            WHEN SV6 IS NOT NULL THEN 'COMPLETED'
                            ELSE 'PENDING'
                        END
                    FROM dbo.TBL_SERVICE_SCEDULE_UPDATE
                    WHERE TECH_CODE = @techcode
                      AND EXPT_SV6 BETWEEN @firstdaymonth AND @lastdayofmonth
                ) x
                GROUP BY
                    MACHINE_REF,
                    MatchedColumn,
                ...
                ";
                DateTime today = GetSriLankanTime();
                DateTime lastdayOfMonth = new DateTime(today.Year, today.Month, 1).AddDays(-1);
                DateTime seconddayOfMonth = new DateTime(today.Year, today.Month, 1).AddMonths(1).AddDays(1);
                var result = connection.Query<ServiceVisitMonthlyInfo>(query, new
                {
                    techcode = techCode,
                    firstdaymonth = lastdayOfMonth,
                    lastdayofmonth = seconddayOfMonth,
                });
        ...
        public async Task<List<ServiceVisitMonthlyInfo>> GetMonthlyVisitCount(string techCode)
        {
            List<ServiceVisitMonthlyInfo> machineCounts = new List<ServiceVisitMonthlyInfo>();
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
